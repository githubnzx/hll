...
class Member extends Base
{
    ...
    public function memberBuy(){
        ...
        if ($pay_type === 1) { // 微信支付
            $data["wxData"] = OrderLogic::getInstance()->payWx($order['code'], $order['present_price'], url('user/pay/notifyWxMemberPay', '', true, true), "亟亟城运司机端购买会员卡");//亟亟城运会员购买
        } else {  // 支付宝支付
            $data['zfbData'] = OrderLogic::getInstance()->payZfb($order['code'], $order['price'], url('user/pay/notifyZfbMemberPay', '', true, true), "亟亟城运司机端购买会员卡");
        }
